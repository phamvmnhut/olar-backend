...
exports.checkCanView = catchAsync(async (request, _response, next) => {
  // admin role
...
exports.checkCanUpdate = catchAsync(async (request, _response, next) => {
  const answer = await Answer.findById(request.params.id).populate('question');
  if (!answer) return next(new AppError('Dont have this answer', StatusCodes.NOT_FOUND));
...
exports.myAnswer = (request, _response, next) => {
  request.query.user = request.user.id;
  return next();
};

exports.convertQueryToQuestionIds = catchAsync(async (req, _res, next) => {
  const questionIds = await Answer.find({ user: req.query.user }).distinct('question');
  req.query._id__in = questionIds.join();
  delete req.query.user;
  return next();
});

exports.setQuestionUserIds = (request, _response, next) => {
  // Allow nested routes
...
exports.restrictUpdateAnswerFields = (request, _response, next) => {
